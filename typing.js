...
function initTyping() {
  ...
  if (charIndex < characters.length - 1 && timeLeft > 0) {
    ...
    // El juego ha terminado, guarda la puntuación
    clearInterval(timer);
    // Guarda la puntuación solo si no se ha guardado antes
    if (!hasSavedScore) {
      saveScore(parseInt(wpmTag.innerText), mistakes); // Guardar la puntuación
      hasSavedScore = true; // Marca como guardado
    }
    inpField.value = ""; // Reiniciar el campo de entrada
  }
}
...
function saveScore(wpm, mistakes) {
  ...
  // Agregar la nueva puntuación a la categoría correspondiente
  rankings[maxTime + "s"].push({ wpm, mistakes });

  // Ordenar los puntajes por WPM de mayor a menor
  rankings[maxTime + "s"].sort((a, b) => b.wpm - a.wpm);

  // Guardar los datos de vuelta en localStorage
...
function showRankings() {
  ...
  // Crear el contenido HTML para los rankings
  let rankingHTML = "<h2>Rankings</h2>";
  for (let category in rankings) {
    rankingHTML += `<h3>${category}</h3><ol>`;
    rankings[category].forEach((score, index) => {
      rankingHTML += `<li>#${index + 1} - WPM: ${score.wpm}, Mistakes: ${
        score.mistakes
      }</li>`;
    });
    rankingHTML += "</ol>";
  ...
  // Mostrar el ranking en algún contenedor de tu página
